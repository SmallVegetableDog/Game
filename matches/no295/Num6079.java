 ...
 * 0 <= discount <= 100
 */
public class Num6079 {

    public static void main(String[] args) {
        Num6079 num6079 = new Num6079();
        System.out.println(num6079.discountPrices("$19 q1zi9oqt m", 0));
    }
    ...
    public String discountPrices(String sentence, int discount) {
        ...
        for (String word : words) {
            ...
            String newWord = discountWord(word.substring(1), discount);
    ...
    private String discountWord(String word, int discount) {
        if (discount == 100) {
            return "$0.00";
        }
        Double num = Double.valueOf(word);
        double div = 1 - (discount / 100.0);
        String format = "$%." + 2 + "f";
        return String.format(format, num * div);
    }

    private boolean isPriceWord(String word) {
        if (word.length() <= 1) {
            return false;
        }
        if (!word.startsWith("$")) {
